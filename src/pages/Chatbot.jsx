    ...
    try {
      ...
      const response = await fetch(API, {
        ...
        body: JSON.stringify({
          ...
          useVoice: false, // désactivé car on utilise détection auto côté backend
        }),
      });

      const data = await response.json();

      // 🖼️ Cas réponse avec image générée
      if (data.imageUrl) {
        setMessages((prev) => [
          ...prev,
      ...
      // 🔊 Cas réponse avec audio généré
      if (data.audioUrl) {
        const fullAudioUrl = `${import.meta.env.VITE_BACKEND_URL}${
          data.audioUrl
        }`;
        setMessages((prev) => [
          ...prev,
      ...
      // 📄 Cas réponse avec fichier PDF
      if (data.fileUrl) {
        const fullFileUrl = `${import.meta.env.VITE_BACKEND_URL}${
          data.fileUrl
        }`;
        setMessages((prev) => [
          ...prev,
      ...
      // 🗨️ Cas réponse texte simple
      else if (data.content && !data.audioUrl) {
        const isCoach = data.source === "coach"; // 👈 vérifie si c’est co
        setMessages((prev) => [
          ...prev,
          { type: isCoach ? "coach-text" : "bot-text", text: data.content },
        ]);
      }
    } catch (err) {
      ...
      setMessages((prev) => [
        ...prev,
        { type: "error", text: "❌ Error communnicating with the server." },
      ]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="chatbot-wrapper">
                        ...
                        {(drag) => (
                          <li
                            ...
                            className={`conversation-item ${
                              activeConversation === c._id ? "active" : ""
                            }`}
                          >
        ...
        <section className="chat-container">
          <div className="chat-messages">
            {messages
              .filter((m, i, arr) => {
                // Si c’est un message coach, on ne garde que s’il est le dernier
                if (m.type === "coach-text") {
            ...
            {loading && (
              <div className="chat-message bot">
                ...
                🤖 Our intelligent assistant, powered by <span>Groq</span>,{" "}
                <span>LLaMA 3</span>, <span>Open IA</span>, and{" "}
                <span>chatbot-agent , coach-agent (uAgents)</span>
                <br />
              </div>
            )}
